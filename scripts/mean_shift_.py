## Changed Version of the Sci-Kit learn code for Mean Shift
###########################################################################
##                                                                        ##
...
def _mean_shift_single_seed(my_mean, X, nbrs, max_iter):
    ...
    while True:
        ...
        # print(my_old_mean, my_old_mean.shape)
        distances = euclidean_distances(points_within, np.reshape(my_old_mean,(1,my_old_mean.shape[0])))
        weights = np.exp(-1 * (distances ** 2 / bandwidth ** 2))
        my_mean=np.sum(points_within * weights, axis=0) / np.sum(weights)
