...
uniform sampler3D uTexVolRGBA; // Volumetric texture - RGBA
...
uniform int uUseDirectRGBARendering;
uniform vec3 uBBox;
...
void main(void) {
    ...
    if (uUseDirectRGBARendering == 1) {
        for(float s = 0; s < delta_t; s += uStepSize) { //Front to back
            texRGBA_sample = texture(uTexVolRGBA, vert2tex(fPosition));
