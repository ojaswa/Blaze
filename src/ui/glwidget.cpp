...
#include <QGLFormat>
#include <QMouseEvent>
#include <depends/OpenGL/OpenGLError>
...
GLWidget::GLWidget(QWidget *parent) : QOpenGLWidget(parent), m_debugLogger(Q_NULLPTR)
{
    ...
    m_useJittering = 0;
    m_useDirectRGBARendering = false;
    m_rgbaVolume = NULL;
...
GLWidget::~GLWidget()
{
    ...
    delete m_trackBall;
    if (m_rgbaVolume) delete []m_rgbaVolume;
}
...
void GLWidget::paintGL()
{
    ...
    m_program->bind();
    {
        ...
        glBindTexture(GL_TEXTURE_3D, m_textureVolRGBA);
        m_program->setUniformValue(m_uTexVolRGBA, 3);
        ...
        m_program->setUniformValue(m_uUseDirectRGBARendering, m_useDirectRGBARendering?1:0);
...
void GLWidget::teardownGL()
{
    ...
    glDeleteTextures(1, &m_textureVolRGBA);
}
...
#undef CASE
    ...
    // qPrintable will print our QString w/o quotes
...
void GLWidget::createVolume(VolumeManager *vm)
{
    ...
    m_uTexVolRGBA = m_program->uniformLocation("uTexVolRGBA");
    ...
    m_uUseDirectRGBARendering = m_program->uniformLocation("uUseDirectRGBARendering");
    ...
    //Generate RGBA volume texture (fill it up later when needed)
    glGenTextures(1, &m_textureVolRGBA);
...
void GLWidget::on_classifiedVolumeComputed()
{
    //Switch to direct RGBA rendering mode
    m_useDirectRGBARendering = true;

    //Update volume
    int width = m_volumeManager->width();
    int height = m_volumeManager->height();
    int depth = m_volumeManager->depth();
    if (m_rgbaVolume == NULL)
        m_rgbaVolume = new unsigned char[4*width*height*depth];

    //Setup texture - destroy the old texture and recreate new (Updating the existing 3D texture does not work on MacOS)
    glDeleteTextures(1, &m_textureVolRGBA);
    glGenTextures(1, &m_textureVolRGBA);
    glBindTexture(GL_TEXTURE_3D, m_textureVolRGBA);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_BORDER);
    ...
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    m_volumeManager->computeRGBAVolume(m_rgbaVolume);
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8,
                 width, height, depth,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, m_rgbaVolume);
    glBindTexture(GL_TEXTURE_3D, 0);
}

void GLWidget::on_classifiedVolumeRead()
{
    //Switch to direct RGBA rendering mode
    m_useDirectRGBARendering = true;
    ...
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    m_volumeManager->computeRGBAVolumeExternal(m_rgbaVolume);
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8,
                 width, height, depth,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, m_rgbaVolume);
    glBindTexture(GL_TEXTURE_3D, 0);
