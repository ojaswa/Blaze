...
protected slots:
    ...
    void on_classifiedVolumeRead();
    void on_volumeGradientComputed();
    void toggleDirectRGBARendering(bool flag) { m_useDirectRGBARendering = flag;}
...
private:
    ...
    GLuint m_textureVolRGBA; // 3D RGBA texture
    ...
    int m_useJittering;
    bool m_useDirectRGBARendering;
    bool m_PerformPhongShading;
    unsigned char* m_rgbaVolume;
    QVector3D m_bbox;

    //Address to uniform variables
    int m_uTexVol, m_uTexTF1D, m_uTexNoise, m_uTexVolRGBA, m_uTexVolNormals;
    int m_uTime, m_uInterpolationType;
    int m_uView, m_uProjection;
    int m_uStepSize;
    int m_uUseJittering;
    int m_uUseDirectRGBARendering;
    int m_uBBox;
